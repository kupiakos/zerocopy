...
pub enum LayoutError {
    ...
    /// The byte slice was not aligned on boundary for `&T`.
    Align,
}
...
pub unsafe trait FromBytes: FromZeroes {
    ...
    // TODO: casting refs will take a trait method in SliceDst
    fn cast_ref_from<'a, B>(bytes: B) -> Result<B::Cast<'a, Self>, LayoutError>
    where
        Self: 'a + Sized,
        B: ReadOnlyByteSlice + 'a,
    {
        todo!()
    }

    // TODO: casting refs will take a trait method in SliceDst
    fn cast_mut_from<'a, B>(bytes: B) -> Result<B::Cast<'a, Self>, LayoutError>
    where
        Self: 'a + Sized,
        B: ByteSliceMut + 'a,
    {
